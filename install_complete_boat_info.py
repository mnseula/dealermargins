...
Handles dynamic SQL with proper quote escaping
"""
...
# Stored procedure SQL with properly escaped quotes
PROCEDURE_SQL = """
...
BEGIN
    ...
    SELECT 
        ...
        Series,
        BoatItemNo,
        CAST(SerialModelYear AS UNSIGNED)
    INTO v_dealer_id, v_series_id, v_model_no, v_model_year
    ...
    -- Get table suffix (20 -> 20, 25 -> 25, 26 -> 26)
    SET v_table_suffix = LPAD(v_model_year - 2000, 2, '0');
    SET v_table_name = CONCAT('BoatOptions', v_table_suffix, '_test');
    ...
    -- Result Set 2: Line Items with Pricing (Dynamic Query)
    SET @sql = CONCAT('
        SELECT 
            ItemNo as item_no,
...
def test_procedure(cursor, hull_no='ETW45922F607'):
    ...
    print("\n--- Result Set 1: Boat Header ---")
    if result_sets[0]:
        headers = ['hull_no', 'model_no', 'dealer_id', 'dealer_name', 'series_raw', 
...
def main():
    ...
    try:
        ...
        # Test procedure with provided hull or default
        hull_no = sys.argv[1] if len(sys.argv) > 1 else 'ETW45922F607'
