...
proc_label: BEGIN
    ...
    -- =====================================================================
    -- Create temp table for MSRP summary
    DROP TEMPORARY TABLE IF EXISTS tmp_msrp_summary;
    CREATE TEMPORARY TABLE tmp_msrp_summary AS
    SELECT 'Base Boat' as category, 'BS1' as category_code, COALESCE(SUM(CASE WHEN product_category = 'BS1' THEN ext_sales_amount END), 0) as msrp FROM tmp_line_items
    UNION ALL SELECT 'Engine', 'EN7', COALESCE(SUM(CASE WHEN product_category IN ('EN7', 'ENG', 'MTR') THEN ext_sales_amount END), 0) FROM tmp_line_items
    UNION ALL SELECT 'Accessories', 'ACC', COALESCE(SUM(CASE WHEN product_category = 'ACC' THEN ext_sales_amount END), 0) FROM tmp_line_items
    UNION ALL SELECT 'Colors/Options', 'H1', COALESCE(SUM(CASE WHEN product_category = 'H1' THEN ext_sales_amount END), 0) FROM tmp_line_items
    UNION ALL SELECT 'Labor/Prep', 'L2', COALESCE(SUM(CASE WHEN product_category = 'L2' THEN ext_sales_amount END), 0) FROM tmp_line_items
    UNION ALL SELECT 'Discounts', 'C1L', COALESCE(SUM(CASE WHEN product_category = 'C1L' THEN ext_sales_amount END), 0) FROM tmp_line_items
    UNION ALL SELECT 'Fees', 'GRO', COALESCE(SUM(CASE WHEN product_category = 'GRO' THEN ext_sales_amount END), 0) FROM tmp_line_items
    UNION ALL SELECT 'TOTAL', 'TOTAL', COALESCE(SUM(ext_sales_amount), 0) FROM tmp_line_items;

    -- =====================================================================
    -- RESULT SET 3: MSRP Summary by Category
    -- =====================================================================
    SELECT
        'Base Boat' as category,
        'BS1' as category_code,
        COALESCE(SUM(CASE WHEN product_category = 'BS1' THEN ext_sales_amount END), 0) as msrp
    FROM tmp_line_items
    UNION ALL
    SELECT
        'Engine' as category,
        'EN7' as category_code,
        COALESCE(SUM(CASE WHEN product_category IN ('EN7', 'ENG', 'MTR') THEN ext_sales_amount END), 0) as msrp
    FROM tmp_line_items
    UNION ALL
    SELECT
        'Accessories' as category,
        'ACC' as category_code,
        COALESCE(SUM(CASE WHEN product_category = 'ACC' THEN ext_sales_amount END), 0) as msrp
    FROM tmp_line_items
    UNION ALL
    SELECT
        'Colors/Options' as category,
        'H1' as category_code,
        COALESCE(SUM(CASE WHEN product_category = 'H1' THEN ext_sales_amount END), 0) as msrp
    FROM tmp_line_items
    UNION ALL
    SELECT
        'Labor/Prep' as category,
        'L2' as category_code,
        COALESCE(SUM(CASE WHEN product_category = 'L2' THEN ext_sales_amount END), 0) as msrp
    FROM tmp_line_items
    UNION ALL
    SELECT
        'Discounts' as category,
        'C1L' as category_code,
        COALESCE(SUM(CASE WHEN product_category = 'C1L' THEN ext_sales_amount END), 0) as msrp
    FROM tmp_line_items
    UNION ALL
    SELECT
        'Fees' as category,
        'GRO' as category_code,
        COALESCE(SUM(CASE WHEN product_category = 'GRO' THEN ext_sales_amount END), 0) as msrp
    FROM tmp_line_items
    UNION ALL
    SELECT
        'TOTAL' as category,
        'TOTAL' as category_code,
        COALESCE(SUM(ext_sales_amount), 0) as msrp
    FROM tmp_line_items;
    ...
    SELECT
        ...
        CASE msrp.category_code
            ...
            WHEN 'TOTAL' THEN (
                SELECT ROUND(SUM(
                    CASE
                        WHEN category_code = 'BS1' THEN msrp * (1 - m.base_boat_margin_pct/100)
                        WHEN category_code = 'EN7' THEN msrp * (1 - m.engine_margin_pct/100)
                        WHEN category_code IN ('ACC', 'H1') THEN msrp * (1 - m.options_margin_pct/100)
                        ELSE msrp
                    END
                ), 2)
                FROM (
                    SELECT 'BS1' as category_code, COALESCE(SUM(CASE WHEN product_category = 'BS1' THEN ext_sales_amount END), 0) as msrp FROM tmp_line_items
                    UNION ALL SELECT 'EN7', COALESCE(SUM(CASE WHEN product_category IN ('EN7', 'ENG', 'MTR') THEN ext_sales_amount END), 0) FROM tmp_line_items
                    UNION ALL SELECT 'ACC', COALESCE(SUM(CASE WHEN product_category = 'ACC' THEN ext_sales_amount END), 0) FROM tmp_line_items
                    UNION ALL SELECT 'H1', COALESCE(SUM(CASE WHEN product_category = 'H1' THEN ext_sales_amount END), 0) FROM tmp_line_items
                    UNION ALL SELECT 'L2', COALESCE(SUM(CASE WHEN product_category = 'L2' THEN ext_sales_amount END), 0) FROM tmp_line_items
                    UNION ALL SELECT 'C1L', COALESCE(SUM(CASE WHEN product_category = 'C1L' THEN ext_sales_amount END), 0) FROM tmp_line_items
                    UNION ALL SELECT 'GRO', COALESCE(SUM(CASE WHEN product_category = 'GRO' THEN ext_sales_amount END), 0) FROM tmp_line_items
                ) calc
            )
            ELSE msrp.msrp
        END as dealer_cost,
