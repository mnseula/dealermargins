...
Partial Class Workstations_FurniturePickList
    ...
    Protected Sub btnRefreshSouth_Click(sender As Object, e As EventArgs) Handles btnRefreshSouth.Click
        gvFurnitureSouth.DataBind()
    End Sub

    Protected Sub gvFurnitureSouth_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvFurnitureSouth.RowCommand
        ...
        If e.CommandName = "gotoupdate2" Then
            ...
            Dim SelectedProdNo As Integer = gvFurnitureSouth.Rows(Key).Cells(0).Text
            Dim img As Image = CType(gvFurnitureSouth.Rows(Key).FindControl("btnUpdateFurniture"), Image)
            Dim floorvalue = img.ImageUrl
    ...
    Protected Sub gvFurnitureSouth_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvFurnitureSouth.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then

            Dim prod As String = e.Row.Cells(0).Text
            prod = Mid(prod, 3)

            Dim s As SqlDataSource = CType(e.Row.FindControl("sdsLocsSouth"), SqlDataSource)
            Try
                s.SelectParameters(0).DefaultValue = prod
            Catch
            End Try
    ...
    Protected Sub locsSouth_RowCommand(sender As Object, e As GridViewCommandEventArgs)
        If e.CommandName = "pullaskid" Then
