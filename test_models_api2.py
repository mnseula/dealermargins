...
if isinstance(data, list) and len(data) > 0:
    # Find M series model
    for rec in data:
        if 'ML' in str(rec.get('value', '')).upper():
            print(f"\nExample M-series model: {rec.get('value')}")
            print("\nAll fields:")
    ...
    for rec in data:
        for key in rec.keys():
            if any(term in key.lower() for term in ['length', 'deck', 'pontoon', 'tube', 'loa', 'beam']):
                length_fields.add(key)
    
    print(f"\n\nLength-related fields found: {sorted(length_fields)}")
