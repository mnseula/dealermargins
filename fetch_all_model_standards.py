"""
Fetch Model Standards for All Series from Infor CPQ Matrix API

This script:
1. Fetches all models from the OptionList API
2. Extracts unique series codes
3. For each series, fetches model standards from Matrix API
...
# Use TRN credentials for Matrix API
...
# Output directory
OUTPUT_DIR = Path("model_standards_data")
...
def fetch_model_standards(token_manager: TokenManager, series: str, year: int = CURRENT_YEAR) -> Optional[Dict]:
    """Fetch model standards for a specific series"""
    matrix_name = f"{series}_ModelStandards_{year}"
    matrix_url = f"{MATRIX_API_BASE}/{matrix_name}/values"

    print(f"\n🔍 Fetching model standards for series: {series}")
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if response.status_code == 404:
                print(f"   ⚠️  Matrix not found (404) - series may not have model standards")
                return None
            ...
            print(f"   ✅ Successfully fetched {total_count} standard feature records")
...
def save_model_standards(series: str, data: Dict, output_dir: Path):
    """Save model standards to JSON file"""
    output_dir.mkdir(parents=True, exist_ok=True)

    filename = output_dir / f"{series}_ModelStandards_{CURRENT_YEAR}.json"
...
def main():
    ...
    print("=" * 80)
    print("FETCH ALL SERIES MODEL STANDARDS FROM CPQ MATRIX API")
    print("=" * 80)
    ...
    # Step 3: Fetch model standards for each series from TRN
    print("\n" + "=" * 80)
    print("FETCHING MODEL STANDARDS FOR EACH SERIES")
    print("=" * 80)
    ...
    for i, series in enumerate(sorted_series, 1):
        ...
        standards_data = fetch_model_standards(trn_token_manager, series, CURRENT_YEAR)

        if standards_data:
            save_model_standards(series, standards_data, OUTPUT_DIR)
            success_count += 1

            # Get record count for summary
            record_count = 0
            if 'result' in standards_data and 'totalDetailsCount' in standards_data['result']:
                record_count = standards_data['result']['totalDetailsCount']
            elif 'result' in standards_data and 'details' in standards_data['result']:
                record_count = len(standards_data['result']['details'])

            results_summary['series_results'].append({
                'series': series,
                'status': 'success',
                'record_count': record_count,
                'matrix_name': f"{series}_ModelStandards_{CURRENT_YEAR}"
            })
        else:
    ...
    print(f"📂 Model standards saved to: {OUTPUT_DIR}")
    print("=" * 80)
