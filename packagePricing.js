...
window.loadPackagePricing = window.loadPackagePricing || function (serialYear, serial, snmInvoiceNo, engineERPNo) {
    ...
    if (serialYear > 4 && serialYear < 8)
    {
        ...
        var whereClause = "WHERE ItemMasterMCT <> 'DIC' AND ItemMasterMCT <> 'DIF' AND ItemMasterMCT <> 'DIP' AND ItemMasterMCT <> 'DIR' AND ItemMasterMCT <> 'DIA' AND ItemMasterMCT <> 'DIW' AND ItemMasterMCT <> 'LOY' AND ItemMasterMCT <> 'PRD' AND ItemMasterMCT <> 'VOD' AND (ItemMasterMCT <> 'DIS' OR (ItemMasterMCT = 'DIS' AND ItemNo = 'NPPNPRICE16S')) AND ItemMasterMCT <> 'DIV' AND ItemMasterMCT <> 'CAS' AND ItemMasterMCT <> 'DIW' AND (ItemMasterMCT <> 'ENZ' OR (ItemMasterMCT = 'ENZ' AND ItemDesc1 LIKE '%VALUE%')) AND ItemMasterMCT <> 'SHO' AND ItemMasterMCT <> 'GRO' AND ItemMasterMCT <> 'ZZZ' AND ItemMasterMCT <> 'FRE' AND ItemMasterMCT <> 'WAR' AND ItemMasterMCT <> 'DLR' AND ItemMasterMCT <> 'FRT' AND (ItemMasterProdCat IS NULL OR ItemMasterProdCat <> '111') ";

        if (snmInvoiceNo && snmInvoiceNo !== '') {
            // Has invoice - use invoice filter
            whereClause += "AND (InvoiceNo = '" + snmInvoiceNo + "' OR (ERP_OrderNo = '" + engineERPNo + "' AND (MCTDesc = 'ENGINES' OR MCTDesc = 'Engine' OR MCTDesc = 'ENGINES IO' OR ItemMasterMCT= 'ELU' OR ItemMasterProdCatDesc = 'EngineLowerUnit'))) ";
            console.log('Using invoice filter:', snmInvoiceNo);
        } else {
            // No invoice - just use serial number
            console.log('No invoice - using serial number only');
        }
    ...
    // CPQ boats have a "Base Boat" line with real MSRP/dealer cost from API
    // We need to save this data before removing the duplicate boat record
    // SAFETY: Initialize to null - if not found, legacy boats use their original pricing
    ...
    if (boatmodel.length > 1) {
        ...
        // First, check if this is a CPQ boat (has "Base Boat" line with MSRP > 0)
        ...
        // CPQ AUTHORIZATION: If this is a CPQ boat and user is not authorized, block access
        if (baseBoatRec.length > 0 && !isCpqAuthorized) {
            console.error('❌ ACCESS DENIED: This is a CPQ boat and you are not authorized to view it.');
            console.error('   Serial: ' + serial);
            console.error('   User: ' + user);
            alert('ACCESS DENIED\n\nThis boat is a CPQ configuration and is not available for your user account.\n\nSerial: ' + serial + '\nPlease contact your administrator if you need access.');
            return; // Stop execution - do not load this boat
        }

        // CPQ AUTHORIZATION: Extract CPQ pricing if user is authorized
        if (isCpqAuthorized && baseBoatRec.length > 0) {
        ...
        } else if (baseBoatRec.length === 0) {
            // No "Base Boat" found - this is a legacy boat with duplicate records
            console.log('No CPQ Base Boat line found - this is a legacy boat');
        }
        ...
        // SAFETY: Only replace boatmodel if we found non-Base Boat records
        if (nonBaseBoats.length > 0) {
            window.boatmodel = nonBaseBoats;
        } else {
            // SAFETY: If filtering would leave us with no boats, keep original array
            console.warn('⚠️ Filtering would remove all boat records - keeping original');
        }
    } else {
        // SAFETY: Single boat record - legacy boat, no extraction needed
        console.log('Single boat record found - using standard pricing (legacy boat)');
    }
